...
def test_should_purchase_places(client, fake_data):
    ...
    response = client.post('/purchasePlaces', data={'competition': competition['name'],
...
def test_should_not_purchase_more_places_than_left(client, fake_data):
    ...
    response = client.post('/purchasePlaces', data={'competition': competition[
        'name'], 'places': 5},
                           follow_redirects=True)
...
def test_should_not_be_able_to_book(client, fake_data):
    ...
    response = client.get(url, data={'competition': competition[
